...
func loadCorpus(corpusDir string, crawlRequestFile string) error {
	...
	var files []string
	var output []byte
	files, err := fileList(corpusDir)
	if err != nil {
		return err
	}
	for _, file := range files {
		log.Println("loadCorpus(): Adding file to ipfs:" + file)
		output, err = (exec.Command("ipfs", "add", file)).Output()
		if err != nil {
			return err
		}
		log.Println(output)
		...
		if strings.Contains(outputstr, "added ") {
			InsertStringToFile(crawlRequestFile, strings.Split(outputstr, " ")[1], -1)
		}
	}
	return nil
}
...
func unzipAll(root string, recursive bool) error {
	...
	for _, file := range zipfiles {
		...
		output, err := (exec.Command("tar", "-xzvf", file)).Output()
		if err != nil {
			return err
		}
		log.Println(output)
